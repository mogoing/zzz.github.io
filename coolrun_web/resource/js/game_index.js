...
function crossAjax(config){
    try{document.domain='u.ali213.net';}catch(e){}
    if(!$iframe){
    ...
    crossAjax({
        ...
        success:function(result){
            ...
            if(data.flag=='f0'||data.flag=='f1'){
                ...
                tem += '</div>';
                tem += '<a ontouchstart="" class="w_button w_button_small fl" id="gobackgame" href="http://u.ali213.net" style="width: 50px; margin-top: 10px;">继续游戏</a>';
...
$j.getJSON('/crossdomain.php?url='+window.location.href,function(result){
	wx.config({
	...
	wx.ready(function(){
		wx.onMenuShareTimeline({
		    ...
		    success: function(res){
		    	if (typeof(mebtnopenurl)!='undefined'){
		    		if (confirm("确定离开，寻找更多游戏？")) document.location.href = mebtnopenurl;
		    	}
		    },
		    cancel: function(res){}
		});
...
//judge whether portrait or landscape
$j(document).ready(function(){
	var uapp = QueryString.getParameter('uapp');
	var ucode = QueryString.getParameter('game_code');
	if (uapp){ //如果是Uapp的话
		$j(".aliForPc").remove();
		var uappwelcome = 0;
		$j(document.body).append('<div id="uappsheild"></div>');
		$j("#uappsheild").css({"position":"absolute","top":0,"left":0,"bottom":"-60px","right":0,"overflow":"hidden","z-index":"2000"}).show();
		$j("#uappsheild").css({"background":"#000 url(/img/uapp/"+ucode+onOrientation()+".jpg) center center no-repeat","background-size":"100% 100%"});
		$j(window).bind('orientationchange',function(event){
			if (uappwelcome == 0) $j("#uappsheild").css({"background":"#000 url(/img/uapp/"+ucode+event.orientation+".jpg) center center no-repeat","background-size":"100% 100%"});
		});
		$j("body #uappsheild").on("tap",function(){
			uappwelcome = 1;
			$j("#uappsheild").hide();
		});
		$j("body").append('<div style="display:none;"><script src="http://s4.cnzz.com/stat.php?id=1253527433&web_id=1253527433" language="JavaScript"></script></div>');
	}
});

function onOrientation(){
	return $j(document).width() > $j(document).height() ? 'landscape' : 'portrait';
}