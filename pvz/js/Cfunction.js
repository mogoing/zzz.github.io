var $User = function() {
	...
	return {
		...
		ES6: function () { // 判断浏览器是否支持ES6
			try { eval ("() => {};"); return true;
			} catch (Reason) { return false; }
		}(), 
		ES8: function () { // 判断浏览器是否支持ES8
			try { eval ("'0721'.padStart(5, '0');"); return true; 
			} catch (Reason) { return false; }
		}(), 
		System: {
			Win: f,
...
oP = {
	Init: function(a) {
		var r = this;
		r.NumZombies = r.FlagZombies = 0, r.FlagMaxWaitTime = 1990, r.FlagZombieWaitTime = 500, r.ZTimeStep = 10, r.AutoStopGame = true; // FlagMaxWaitTime: 两波之间最大间隔; FlagZombieWaitTime: 两波之间最小间隔; ZTimeStep: 僵尸移动的间隔; AutoStopGame: 是否暂停游戏
		if (a) {
			var l;
			for (l in a) {
				r[l] = a[l]
		...
		a && a.FlagNum ? (r.FlagHeadStep = 140 / (a.FlagNum - 1), r.MonPrgs = function() {
			...
			if (i < u.FlagNum) {
				...
				oP.AutoStopGame && (NewEle("DivA", "div", "position:absolute;width:900px;height:600px;background:#FFF;filter:alpha(opacity=0);opacity:0;z-index:255", 0, EDAll), PauseGame($("dMenu0"), 1));
				u.FlagToEnd();
				$User.isAuthorWebsite && $User.Visitor.UserName != "游客" && ClearChild($("JSPVZAjax"));
				f.SaveLvl && NewEle ("JSPVZAjax", "script", 0, {
						src: "asp/UserSave.asp?Lvl=" + (s = oS.LvlEName) + "&T=" + (oSym.Now - oS.StartTime),
						type: "text/javascript"
					}, document.body);
				f.SaveLvlCallBack && f.SaveLvlCallBack({
						UserName: f.UserName,
						SunNum: oS.SunNum,
						Lvl: s,
						T: oSym.Now - oS.StartTime
					});
				if (!isNaN(Math.floor(s))) {
					t = $("dAdventure"), $User.Visitor.Progress = ++s, t.firstChild.innerHTML = Math.ceil(s / 10);
					t.childNodes[1].innerHTML = (s = s - Math.floor(s / 10) * 10) ? s : s + 1;
				}
			}
	...
	SelectFlagZombie: function(j, d) {
		...
		c = false,
		h = !n ? oS.NormalFlagZombieTask : (PlayAudio("siren"), n.style.top = "5px", --j, i[g++] = oS.FlagZombie || oFlagZombie, oS.BigFlagZombieTask),
		p,
		b,
		f = e.MustShowAtFlag,
		a;
	...
	SelectFlagZombie1: function(d) {
		...
		e = h.SumToZombie,
		b = !f ? oS.NormalFlagZombieTask : (f.style.top = "5px", --d, c[a++] = oS.FlagZombie || oFlagZombie, oS.BigFlagZombieTask);
		while (d > 0) {
	...
	FlagPrgs: function() {
		...
		[b]), oSym.addTask(oP.FlagMaxWaitTime,
		function(g) {
			var h = oP;
	...
	Monitor: function(a, b) {
		...
		oP.UserDefinedFlagFunc = b ? b: null; (function() {
			oZ.traversalOf();
			oSym.addTask(oP.ZTimeStep, arguments.callee, [])
		})()
	}
},
oGd = {
...
KeyBoardGrowPlant = function(b, a) {
	...
	if (b > 47 && b < 58) {
		switch (a) {
		case 0:
			ChosePlant({ clientX: 450, clientY: 300 }, (b - 48 - 1 + 10) % 10)
		}
	}
},
...
AutoSelectCard = function() {
	...
	b = -1,
	a = c.length;
	while (++b < a) if (c[b].prototype.CanSelect) SelectCard(c[b].prototype.EName);
},
InitPCard = function() {
	var d = "",
	f, e = oS.ArCard,
...
LetsGO = function() {
	...
	while (g < k) { (function(d) {
			f = (j = e.childNodes[d]).id.substr(5).split("_$_")[0];
...
ChosePlant = function(h, d) {
	...
	if (!g.CDReady || !g.SunReady) return;
	h = window.event || h;
	...
	f, c = j.PicArr;
	// !oS.CardKind 
	oS.Chose = 1; (j.name == "Plants") ? EditImg(NewImg("MovePlant", c[j.StaticGif], "left:" + b - 0.5 * (j.beAttackedPointL + j.beAttackedPointR) + "px;top:" + a + 20 - j.height + "px;z-index:254", EDAll).cloneNode(false), "MovePlantAlpha", "", {
		visibility: "hidden",
...
PrepareGrowPlants = function(a) {
	...
	oS.HaveFog && oS.AutoMoveFog ? oGd.MoveFogLeft(b) : b()
},
...
CheckLogin = function(inx) {
	...
	a = /^\w{3,10}$/,
	b = /^[\u4e00-\u9fa5\w]{3,10}$/,
	d = !inx? /^[\dA-Za-z\@\$\!\%\*\#\?\&\^\_]{3,20}$/ : /^(?=.*[A-Za-z])(?=.*\d)[\dA-Za-z\@\$\!\%\*\#\?\&\^\_]{6,20}$/; //密码为6-20位，必须有数字+字母，可以有特殊字符
	return (a.exec(c) || b.exec(c)) && d.test(e) ? true: false
},
...
AppearTombstones = function(n, e, m) {
	var r = oGd.$Tombstones, k = [], h = oS.R + 1, b, d = 0, q, a, g, f, s = oGd.$, l, p;
	while (--h) {
		g = e;
